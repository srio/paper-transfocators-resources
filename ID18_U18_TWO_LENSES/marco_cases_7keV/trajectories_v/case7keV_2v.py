...
if __name__ == "__main__":
    ...
    F1 = numpy.linspace(15,100,200) # numpy.linspace(15,60,3) # numpy.array([41.69]) #
    R1 = []
    for F in F1:
    ...
    for i in range(len(R1)):
        ...
        tally.plot_spectral_density(show=0,filename="%s_spectral_density_%03d.png" % (fileroot, i), title="R1: %g" % F1[i])
        # tally.plot_occupation(show=0,filename="%s_occupation.png" % fileroot)

        tally.save_spectral_density(filename="%s_spectral_density_%03d.dat" % (fileroot,i))
        os.system("echo %03i >> tmp.txt" % i)
        os.system("echo %g >> tmp.txt" % R1[i])
        os.system("echo %g >> tmp.txt" % F1[i])
        os.system("mv tmp.txt %s_%03d.txt" % (fileroot, i) )

        tally.save_occupation(filename="%s_occupation_%03d.dat" % (fileroot, i))
