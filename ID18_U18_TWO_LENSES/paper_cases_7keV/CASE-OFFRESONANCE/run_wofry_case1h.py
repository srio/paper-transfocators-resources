...
def run_source(my_mode_index=0, energy=7000.0):
...
def run_beamline(output_wavefront):
    ...
    boundary_shape = Rectangle(-2.015e-05, 2.015e-05, -2.015e-05, 2.015e-05)
...
def main(energy=7000):
    ...
    for my_mode_index in range(50):
        ...
        tally_source.append(output_wavefront)
    ...
    tally.save_spectral_density(filename="spectral_density.dat")
    tally.save_occupation(filename="occupation.dat")

    tally_source.save_spectral_density(filename="spectral_density_source.dat")
    tally_source.save_occupation(filename=            "occupation_source.dat")
...
for energy in Energy:
    main(energy)
    command = "mv spectral_density.dat case1h/sample_spectral_density_%4d.dat" % energy
    print(command)
    os.system(command)
    command = "mv occupation.dat case1h/sample_occupation_%4d.dat" % energy
    print(command)
    os.system(command)

    command = "mv spectral_density_source.dat case1h/source_spectral_density_%4d.dat" % energy
    print(command)
    os.system(command)
    command = "mv occupation_source.dat case1h/source_occupation_%4d.dat" % energy
    print(command)
    os.system(command)