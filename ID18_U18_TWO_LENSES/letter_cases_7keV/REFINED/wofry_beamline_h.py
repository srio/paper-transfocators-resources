...
def run_source(my_mode_index=0, load_from_file=True, write_to_file=False):
    global coherent_mode_decomposition

    if load_from_file:
        return  GenericWavefront1D.load_h5_file("source_modes_h.h5", filepath="wfr%03d" % my_mode_index)
    ...
    if write_to_file:
        if my_mode_index == 0:
            ...
            overwrite = 0
        output_wavefront.save_h5_file("source_modes_h.h5",subgroupname="wfr%03d" % my_mode_index,
                                      intensity=True, phase=False, overwrite=overwrite, verbose=True)
...
def run_beamline(output_wavefront, aperture, r1, r2):
    ...
    #self.set_additional_parameters(propagation_parameters)
    #
    propagation_parameters.set_additional_parameters('magnification_x', 0.25) #, 0.5)
    propagation_parameters.set_additional_parameters('magnification_N', 1.0)
    #
    propagator = PropagationManager.Instance()
    try:
