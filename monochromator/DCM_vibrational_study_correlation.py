...
def run_loop(sigma1_urad = 0.0, sigma2_urad = 0.0, DCM_pos=38, corr=0.0):
    ...
    #correlation
    ss1 = numpy.array([-sigma1_deg, sigma1_deg])
    ss2 = numpy.array([-sigma2_deg, sigma2_deg])

    means = [ss1.mean(), ss2.mean()]
    stds = [ss1.std(), ss2.std()]
    ...
    #rutine to get ramdom normal distributions with a correlation
    r = numpy.random.multivariate_normal(means, covs, nruns).T    
    r1 = r[0]
    r2 = r[1]
...
if __name__  == "__main__":
    ...
    nruns = 200
    corrs = numpy.round(numpy.linspace(-1.0, 1.0, 7, endpoint=True),1)
    #nruns = 2
    ...
    for corr in corrs:
        sizes1, hx1, hy1 = run_loop(sigma1_urad=0.5, sigma2_urad=0.5, DCM_pos=DCM_pos, corr=corr)
        df[f'RMS_{corr}'] = sizes1
    #sizes0, hx0, hy0 = run_loop(sigma1_urad=0.0, sigma2_urad=0.0, DCM_pos=DCM_pos, corr=corr)
    #sizes1, hx1, hy1 = run_loop(sigma1_urad=0.5, sigma2_urad=0.5, DCM_pos=DCM_pos)
    #sizes1, hx1, hy1 = run_loop(sigma1_urad=2.0, sigma2_urad=2.0)
    #sizes2, hx2, hy2 = run_loop(sigma1_urad=1, sigma2_urad=1, DCM_pos=DCM_pos)
    #sizes2, hx2, hy2 = run_loop(sigma1_urad=4.0, sigma2_urad=4.0)

    #data = {'distances':distances,'0 urad RMS':sizes0,'0.5 urad RMS':sizes1,'1 urad RMS':sizes2}

    #df = pd.DataFrame(data)

    df.to_csv(f'DCM_vibs_dist_to_focal_point_pos_{DCM_pos}_corr_study.csv')
